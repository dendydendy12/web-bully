...
<html>
	...
	<body class="font-[Poppins] pb-[72px]">
		...
		@foreach ($categories as $category)
			...
			<a href="{{route('front.category', $category->name)}}" class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18]">
				<div class="w-6 h-6 flex shrink-0">
		...
		<section id="Up-to-date" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			...
			@forelse ($articles as $article)
				<a href="{{route('front.details', $article->slug)}}" class="card-news">
					...
					@if($article->category)
 						  <p class="badge-white absolute top-5 left-5 rounded-full p-[8px 18px] bg-white font-bold text-xs leading-[18px]">{{$article->category->name}}</p>
						@else
    					<p class="badge-white absolute top-5 left-5 rounded-full p-[8px 18px] bg-white font-bold text-xs leading-[18px]">Kategori tidak ditemukan</p>
							@endif
				...
				<a href="{{route('front.author', $author->slug)}}" class="card-authors">
					<div class="rounded-[20px] border border-[#EEF0F7] p-[26px_20px] flex flex-col items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
						<div class="w-[70px] h-[70px] flex shrink-0 rounded-full overflow-hidden">
							<img src="{{Storage::url($author->photo)}}" class="object-cover w-full h-full" alt="avatar" />
						</div>
						<div class="flex flex-col gap-1 text-center">
							<p class="font-semibold">{{$author->name}}</p>
						@if($author && $author->news)
    					{{ $author->news->count() }}
						@else
    					<p>Belum ada data</p>
						@endif
...
@if($bullying_featured_articles)
    <p class="text-white">Featured</p>
    <a href="details.html" class="font-bold text-[30px] leading-[54px] text-white">
